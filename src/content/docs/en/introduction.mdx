...
1. Use the search bar to search for key words or phrases. Surround multiple words with quotes to search for an exact phrase.
2. Once you've found a video, use your browser's builtin search function (eg. CTRL+F on desktop, 3 dot menu -> Find in page on chrome mobile) to quickly navigate through the transcript.
3. Hover over a line on the transcript to see its timestamp information. If a video is available, clicking on the line will jump the video to the corresponding start time.
